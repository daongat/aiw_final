			...
			<div class="col-lg-8">
				...
				<div class="latest_news">
					@foreach($category_posts as $category_post)
						<div class="media">
							<div class="d-flex">
								<img class="img-fluid" src="{{asset('')}}img/post/{{$category_post->thumbnail}}" alt="" style="width: 400px; height: 250px">
							</div>
							<div class="media-body">
								<div class="choice_text">
									<div class="date">
										<a class="gad_btn" href="#">{{$category_post->category}}</a>
										<a href="#"><i class="fa fa-calendar" aria-hidden="true"></i>{{$category_post->date}}</a>
										<a href="#"><i class="fa fa-eye" aria-hidden="true"></i>{{$category_post->view_count}}</a>
									</div>
									<a href="{{route('articles.detail',$category_post->slug)}}"><h4>{{$category_post->title}}</h4></a>
									<p>{{$category_post->short_des}}</p>
								</div>
							</div>
						</div>
					@endforeach
